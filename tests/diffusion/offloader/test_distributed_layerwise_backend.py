...
def patched_offload_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        dist_backend_module.current_omni_platform, "current_stream", lambda: DummyStream()
    )
...
class TestDistributedLayerwiseOffloadHook:
    ...
    def test_dtensor_wrapper_preserved_across_prefetch_and_offload(
        self, dist_group, patched_offload_runtime
    ):
