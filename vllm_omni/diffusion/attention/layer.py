...
class Attention(nn.Module):
    ...
    def _run_iterative_local_attention(
        ...
        # Stream output to CPU per head group to keep peak GPU memory at
        # O(group_size * S * head_dim) instead of O(num_heads * S * head_dim).
        # Q/K/V are passed in by the caller and remain resident regardless.
        device = query.device
        cpu_slices: list[torch.Tensor] = []
        ...
        out = torch.cat(cpu_slices, dim=2).to(device, non_blocking=True)
        return out
