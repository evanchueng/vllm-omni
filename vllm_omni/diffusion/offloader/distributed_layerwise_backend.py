        ...
        if self.dp_size <= 1 or self.dp_group is None:
            ...
            with current_omni_platform.stream(self.comm_stream):
                for dtype, local_shard in gpu_shards.items():
                    ...
                    # Use synchronous AllGather — HCCL's async_op=True does
                    # not guarantee stream synchronization on NPU, causing
                    # the event to fire before the collective completes,
                    # leading to partial weights and garbled output.
...
class DistributedLayerwiseOffloadBackend(OffloadBackend):
    ...
    def _register_on_demand_hook(self, module: nn.Module, label: str) -> None:
        ...
        CPU after forward.  Keeps VAE/encoders off GPU during the DiT loop.
        They are only needed briefly for encode/decode.
        """
        device = self.device
